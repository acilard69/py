...
import time, random, asyncio, timeit, sys, json, codecs, threading, glob, re, string, os, requests, subprocess, six, urllib, urllib.parse, ast, pytz, wikipedia, pafy, youtube_dl, atexit, requests
...
client = LINE()
#client = LINE(authToken="EDqFcgA5vTwxQ6mLpvG0.CJvp9xSifo2zIbrt0XX+Sa.lAODNoA2ZN56pxSmQ2VNfN4LAYrpUdT89J14duBpfPo=")
...
timeline = client.getHomeProfile()
...
Admin=[myMID]
Owner=["u72354ff57b0814ed99881453d31c7fb0"]
...
Connect_to = {
    ...
    "Copy":False,
    "autoAdd":False,
    "MentionText":"Dont Tag Me!Me is bussy",
    ...
    "Admin": {
        "u72354ff57b0814ed99881453d31c7fb0":True #MID ADMIN TARO DISINI
    },
    "mimic": {
        "copy": False,
        "conpp": False,
...
agent = {'User-Agent' : "Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.84 Safari/537.36"}    
                ...
                if "MENTION" in msg.contentMetadata.keys() != None:
                        ...
                        for mention in mentionees:
                            if myMID in mention["M"]:
                                if send not in Connect_to["userMentioned"]:
                                    Connect_to["userMentioned"][send] = {}
                                if man not in Connect_to["userMentioned"][send]:
                                    Connect_to["userMentioned"][send][man] = 1
                                else:
                                    Connect_to["userMentioned"][send][man] = Connect_to["userMentioned"][send][man] + 1
                ...
                if msg.contentType == 0:
                    if Connect_to["autoRead"] == True:
                        ...
                        if msgText.lower() == '_PYTHON3_':
                            ...
                            if man in Team or man in Connect_to["Admin"]:
                                no = time.time()
                                client.sendText("u902d0c48d5a14ab6ed00d047f16a39af", ' ')
                        ...
                        elif msgText.lower() == "siapatag":
                            if send in Connect_to["userMentioned"]:
                                if Connect_to["userMentioned"][send] != {}:
                                    ret_ = "╭════[ Detect Mention Me]"
                                    no = 0
                                    for lu in Connect_to["userMentioned"][send]:
                                        contact = client.getContact(lu)
                                        conn = client.getContact(lu)
                                        ret_ += "\n║ {}. {} #{}x".format(str(no), str(contact.displayName), str(Connect_to["userMentioned"][send][lu]))
                                    ret_ += "\n╰════[ Total Detect {} ]".format(str(len(Connect_to["userMentioned"][send])))
                                    client.sendText(send, str(ret_)) 
                                    client.arifistifik(send,conn.mid,"ada apa ngetag? ","")
                                else:
                                    client.sendText(send, "Tidak ada yang ngetag")
                            else:
                                client.sendText(send, "Tidak ada yang ngetag")
                            ...
                            if man in Team or man in Connect_to["Admin"]:
                                ...
                                client.sendText(send,"Team\n\nOwner:\n"+msg+"\nAdmin:\n"+dpk_+"\n( %s ) TEAM Croot" %(str(len(Owner)+len(Connect_to["Admin"]))))                                
                              ...
                              if msg.toType == 2:
                                    ...
                                    if jmlh <= 1000:
                                      for x in range(jmlh):
                                    	...
                                    	client.sendMessage(send ,"Jumlah melebihi batas")
                        ...
                        elif msgText.lower() == 'mention':
                            group = client.getGroup(send)
                            nama = [contact.mid for contact in group.members]
                            nm1, nm2, nm3, nm4, nm5, jml = [], [], [], [], [], len(nama)
                            if jml <= 100:
                              mentionMembers(send, nama)                

                            elif jml > 100 and jml < 200:
                                for i in range(0,100):
                                    nm1 += [nama[i]]
                                mentionMembers(send, nm1)
                                for j in range(100, len(nama)-1):
                                    nm2 += [nama[j]]
                                mentionMembers(send, nm2)
                                ...
                                for y in range(300, 400):
                                    nm4 += [nama[y]]
                                mentionMembers(send, nm4)
                                for z in range(400, len(nama)-1):
                                    nm5 += [nama[z]]
                                mentionMembers(send, nm5)
                                print ("SuccessBosQMentionAll")
                        ...
                        elif msgText in ["Autorespon on"]:
                          if man in Team or man in Connect_to["Admin"]:
                            if man in Team:
                                Connect_to['AutoRespon'] = True
                                client.sendText(send,"AutoRespon Already ON")
                        elif msgText in ["Autorespon off"]:
                          if man in Team or man in Connect_to["Admin"]:
                            if man in Team:
                                Connect_to['AutoRespon'] = False
                                client.sendText(send,"AutoRespon Already Off")  
                        elif msgText in ["Autoread on"]:
                          if man in Team or man in Connect_to["Admin"]:
                            if man in Team:
                                Connect_to['autoRead'] = True
                                client.sendText(send,"Auto Already ON")
                        elif msgText in ["Autoread off"]:
                          if man in Team or man in Connect_to["Admin"]:
                            if man in Team:
                                Connect_to['autoRead'] = False
                                client.sendText(send,"Auto Already Off")                                 
                            ...
                            if man in Team or man in Connect_to["Admin"]:
                                ...
                                client.sendText(send, "Reject Set To On..")
                        elif msgText in ["Autoreject off"]:
                            if man in Team or man in Connect_to["Admin"]:
                                Connect_to['AutoReject'] = False
                                client.sendText(send, "Reject Set To Off..")
                          ...
                          if man in Team or man in Connect_to["Admin"]:
                            ...
                            if man in Team or man in Connect_to["Admin"]:
                                ...
                                with requests.session() as s:
                                    s.headers["User-Agent"] = random.choice(Connect_to["userAgent"])
                                    r = s.get(url)
                                    data = r.text
                                    data = json.loads(data)
                                    cu = "Urban Result\n\n"
                                    ...
                                    client.sendMessage(send, str(cu))
                            ...
                            if man in Team or man in Connect_to["Admin"]:
                                ...
                                try:
                                    ...
                                    hasil = timeline.getHomeProfile(mid=a)
                                    st = hasil['result']['feeds']
                                    for i in range(len(st)):
                                        test = st[i]
                                        result = test['post']['postInfo']['postId']
                                        client.like(str(man), str(result), likeType=random.choice(typel))
                                        client.comment(str(man), str(result), 'Auto Like by xxx')
                                    client.sendText(send, 'Done Like+Comment '+str(len(st))+' Post From' + str(s))
                                except Exception as e:
                                    client.sendText(send, str(e))
                            ...
                            if man in Team or man in Connect_to["Admin"]:
                                ...
                                with requests.session() as web:
                                    ...
                                    msgText = web.get("http://api.corrykalam.net/apiloc.php?lokasi={}".format(urllib.parse.quote(location)))
                                    data = msgText
                                    data = json.loads(data)
                                    ...
                                    client.sendMessage(send,str(ret_))
                            ...
                            if man in Team or man in Connect_to["Admin"]:
                                ...
                                with requests.session() as web:
                                    web.headers["man-agent"] = "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36"
                                    msgText = web.get("http://api.corrykalam.net/apisholat.php?lokasi={}".format(urllib.parse.quote(shalat)))
                                    data = msgText
                                    data = json.loads(data)
                            ...
                            try:
                                sep = msgText.split(" ")
                                textToSearch = msgText.replace(sep[0] + " ","")
                                ...
                                client.sendVideoWithURL(send, me)
                            except Exception as e:
                                client.sendMessage(send,str(e))
                        elif msgText.lower().startswith("image: "):
                            googl = msgText.replace('Image: ',"")
                            ...
                            try:
                                start = timeit.timeit()
                                client.sendImageWithURL(send,path)
                                client.sendMessage(send, "Google Image \nType: Search Image\nWaktu dicari: %s" % (start) +"\nTotal Image Links = "+str(len(items)))
                                print ("[Notif] Search Image Google Sucess")
                            except Exception as e:
                                client.sendMessage(send, str(e))
                        elif msgText.lower().startswith("get-igpost: "):
                            sep = msgText.split(" ")
                            ig = msgText.replace(sep[0] + " ","")
                            username = ig[0]
                            urutan = ig[1]
                            r = requests.get('http://rahandiapi.herokuapp.com/instapost/'+username+'/'+urutan+'?key=betakey', verify=True)
                            start = timeit.timeit()
                            data=r.text
                        ...
                        elif msgText.lower().startswith("ig: "):
                            cari = msgText.replace("ig: ","")
                            try:
                                response = requests.get("https://www.instagram.com/"+cari+"?__a=1")
                                data = response.json()
                                namaIG = (data['user']['full_name'])
                                bioIG = (data['user']['biography'])
                                mediaIG = (data['user']['media']['count'])
                                verifIG = (data['user']['is_verified'])
                                usernameIG = (data['user']['username'])
                                followerIG = (data['user']['followed_by']['count'])
                                profileIG = (data['user']['profile_pic_url_hd'])
                                privateIG = (data['user']['is_private'])
                                followIG = (data['user']['follows']['count'])
                                Url = (data['user']['external_url'])
                                text = "StalkIG: "+namaIG +"\n==============================\n[Name] : "+namaIG+"\n[Biography] :\n"+bioIG+"\n[Follower] : "+followerIG +"\n[Following] : "+followIG+"\n[Media] : "+mediaIG+"\n[Verified] :"+verifIG+"\n[Private] : "+privateIG+"\nLinkID: instagram.com/"+usernameIG+"\nURL: "+Url+"\n==============================\n\nStalkIG: " +namaIG+"\nHas Been Clear"
                                client.sendImageWithUrl(send, profileIG)
                                client.sendText(send, str(text))
                            except Exception as e:
                                client.sendText(send, str(e))
                        elif msgText.lower().startswith("igpost "):
                            user = msgText.replace("igpost ","")
                            query = '1'
                            with requests.session() as s:
                                s.headers['user-agent'] = 'Mozilla/5.0'
                                url    = 'https://www.instagram.com/'+user+'/'
                                params = {'__a': query}
                                r      = s.get(url, params=params).content
                                data = json.loads(r)['user']
                                user_id   = data['id']
                                with requests.session() as s:
                                    s.headers['cookie']  = 'diisi cookie sndiri'
                                    s.headers['user-agent'] = 'Instagram 10.3.2 (iPhone7,2; iPhone OS 9_3_3; en_US; en-US; scale=2.00; 750x1334) AppleWebKit/420+'
                                    s.headers['cache-control'] = 'no-cache'
                                    s.headers['x-ig-capabilities'] = '36oD'
                                    respon = s.get('https://i.instagram.com/api/v1/feed/user/'+user_id+'/story/')
                                    data = json.loads(respon.content.decode('utf-8'))['reel']
                                    if data == None:
                                        client.sendMessage(send, "Story not found.")
                                    else:
                                        for i in range(len(data)):
                                            if 'video_versions' in data['items'][i]:
                                                video = data['items'][i]['video_versions'][0]['url']
                                                client.sendVideoWithUrl(send, video)
                                            else:
                                                medias = data['items'][i]['image_versions2']['candidates'][0]['url']
                                                client.sendImageWithUrl(send, medias)
                        elif msgText.lower().startswith("imagetext "):
                            sep = msgText.split(" ")
                            textnya = msgText.replace(sep[0] + " ","")
                           ...
                           #urllib.urlretrieve(path, "steal.png")
                            client.sendImage(send,"steal.png")
                        elif "meme " in msgText.lower():
                            data = msgText.lower().replace("meme ","")
                            meme = data.split('|')
                            ...
                            client.sendImageWithURL(send,memes)                


                        elif "musik: " in msgText.lower():
                            try:
                                sep = msgText.split(" ")
                                textToSearch = msgText.replace(sep[0] + " ","")
                                query = urllib.parse.quote(textToSearch)
                                ...
                                for s in stream:
                                    ...
                                    rating = '\n⏩Rating : ' + str(vid.rating)
                                    deskripsi = '\n⏩Deskripsi : ' + str(vid.description)
                                client.sendText(send,"tunggu sebentar yah lagi proses ...")
                                client.sendText(send,"Tunggu sampai lagu nya muncul ya kak:)\n baru perintah lagi")
                                client.sendAudioWithURL(send, shi)
                            except Exception as e:
                                client.sendText(send,str(e))
...
def autoSta():
    ...
    while True:
        try:
           for posts in client.activity(limit=200)['result']['posts']:
             if posts["postInfo"]["liked"] is False:
                if Connect_to["likeOn"] == True:
                   client.likePost(posts["userInfo"]["writerMid"], posts["postInfo"]["postId"], likeType=1001)
                   if Connect_to["commentOn"] == True:
                      if posts["userInfo"]["writerMid"] in Connect_to["commentBlack"]:
                         pass
                      else:
                          client.createComment(posts["userInfo"]["writerMid"],posts["postInfo"]["postId"],Connect_to["comment"])
        except:
            count += 1
            if(count == 50):
                sys.exit(0)
